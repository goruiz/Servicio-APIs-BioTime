...
class MarcacionesDto(BaseModel):
    ...
    gps_location: str = Field(..., description="Ubicación GPS")
    mobile: Optional[str] = Field(None, description="Número de móvil")
    source: int = Field(..., description="Fuente de la marcación")
    purpose: int = Field(..., description="Propósito de la marcación")
    crc: Optional[str] = Field(None, description="CRC de la marcación")
    is_attendance: int = Field(..., description="Indica si es una marcación de asistencia")
    reserved: Optional[str] = Field(None, description="Campo reservado")
    upload_time: str = Field(..., description="Fecha y hora de subida")
    sync_status: int = Field(..., description="Estado de sincronización")
    sync_time: Optional[str] = Field(None, description="Fecha y hora de sincronización")
    emp: Optional[EmployeeDto] = Field(None, description="Información del empleado")
